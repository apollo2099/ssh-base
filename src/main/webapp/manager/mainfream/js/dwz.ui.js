...
function initUI(_box){
	...
	$("a[target=navTab]", $p).each(function(){
		$(this).click(function(event){
			...
			//yangzheng start 如果target=navTab 存在扩展属性closeOthers=true 则做如下处理，使得在打开table页时先关闭掉其他打开着的table页。
			var closeOthers = eval($this.attr("closeOthers") || "false");
			
            //yangzheng end 如果target=navTab 存在扩展属性closeOthers=true 则做如下处理，使得在打开table页时先关闭掉其他打开着的table页。
			
			navTab.openTab(tabid, url,{title:title, fresh:fresh, external:external,closeOthers:closeOthers});
	...
	if ($.fn.dwzExportString) $("a[target=dwzExportString]", $p).dwzExportString($p);
	...
	if ($.fn.dwzSelectedTodo) $("a[target=dwzSelectedTodo]", $p).dwzSelectedTodo();
	if ($.fn.dwzDialog) $("a[target=dwzDialog]", $p).dwzDialog();
