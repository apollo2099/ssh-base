	...
	$.fn.extend({
		...
		suggest: function(){
			...
			return this.each(function(){
				var $input = $(this).attr('autocomplete', 'off').keydown(function(event){
					if (event.keyCode == DWZ.keyCode.ENTER) return false; //灞忚斀鍥炶溅鎻愪氦
				});
		...
		itemDetail: function(){
			...
			function tdHtml(field){
				...
				switch(field.type){
					case 'del':
						html = '<a href="javascript:void(0)" class="btnDel '+ field.fieldClass + '">鍒犻櫎</a>';
						break;
					case 'lookup':
						var suggestFrag = '';
						...
						html = '<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'
							...
							+ '<a class="btnLook" href="'+field.lookupUrl+'" lookupGroup="'+field.lookupGroup+'" '+suggestFrag+' lookupPk="'+field.lookupPk+'" title="鏌ユ壘甯﹀洖">鏌ユ壘甯﹀洖</a>';
						break;
					case 'attach':
						html = '<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'
							+ '<input type="text" name="'+field.name+'" size="'+field.size+'" readonly="readonly" class="'+field.fieldClass+'"/>'
							+ '<a class="btnAttach" href="'+field.lookupUrl+'" lookupGroup="'+field.lookupGroup+'" '+suggestFrag+' lookupPk="'+field.lookupPk+'" width="560" height="300" title="鏌ユ壘甯﹀洖">鏌ユ壘甯﹀洖</a>';
						break;
					case 'enum':
						$.ajax({
					...
					case 'date':
						html = '<input type="text" name="'+field.name+'" value="'+field.defaultVal+'" class="date '+field.fieldClass+'" format="'+field.patternDate+'" size="'+field.size+'"/>'
							+'<a class="inputDateButton" href="javascript:void(0)">閫夋嫨</a>';
						break;
					default:
			...
			return this.each(function(){
				...
				$this.click(function(){
					...
					function _dwzExport(){
						...
						window.location.href=	 url+(url.indexOf('?') == -1 ? "?" : "&")+"ids="+ids;
						if($this.attr("isref")==1){
						  navTabPageBreak();
					...
					if (title) {
						alertMsg.confirm(title, {okCall: _dwzExport});
					} else {
						_dwzExport();
					}
					return false;
		...
		dwzExportString: function(options){
		...
		dwzDialog: function(options){
			...
			return this.each(function(){
				...
				$this.click(function(){
					...
					function _dwzDialog(){
						var url=$this.attr('href');
						url=url+(url.indexOf('?') == -1 ? "?" : "&")+"ids="+ids;
						//$.pdialog.open(url, "_blank", $this.attr("title") || $this.text(), {mask:true,width:520,height:260});
						//$.pdialog.open(url, "_blank", $this.attr("title") || $this.text(), options);
						$.pdialog.open(url, "", $this.attr("title") || $this.text(), options);
					}
					
					var title = $this.attr("title");
					if (title) {
						alertMsg.confirm(title, {okCall: _dwzDialog});
					} else {
						_dwzDialog();
					}
					return false;
